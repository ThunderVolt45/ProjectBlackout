...
void UBlackoutGA_MeleePlayer::OnComboInputPressed(float TimeWaited)
{
	...
	// 콤보 입력이 감지되었으므로 이동 캔슬 타이머를 즉각 해제하여 예기치 못한 취소를 미연에 방지합니다.
...
void UBlackoutGA_MeleePlayer::OnAbilityCancelableEventReceived(FGameplayEventData Payload)
{
	...
	// 연속 공격(콤보) 입력을 유예하기 위해 0.12초(120ms) 후에 이동 입력 감지 타이머를 기동합니다.
	if (UWorld* World = GetWorld())
	{
...
void UBlackoutGA_MeleePlayer::CheckCancelInput()
{
	...
	// 콤보 입력이 대기 중이거나 이미 큐에 쌓여 있다면 강제 캔슬을 방지하고 콤보 전이를 양보합니다.
	if (bComboInputQueued || bHasQueuedComboInputPayload)
	{
		return;
	}
	...
	{
		// 유예 시간이 지난 시점에도 여전히 이동 입력(WASD)이 들어가 있다면 강제 정지 및 복구를 수행합니다.
